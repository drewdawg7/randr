...
fn handle_upgrade_item(
    ...
    for event in upgrade_events.read() {
        ...
        let Some(inv_item) = player.find_item_by_uuid(event.item_uuid) else {
            continue;
        };
        ...
        if let Some(inv_item_mut) = player.find_item_by_uuid_mut(event.item_uuid) {
