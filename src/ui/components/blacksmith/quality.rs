...
const STONES_REQUIRED_PER_UPGRADE: u32 = 1;
...
fn rebuild_items(item_list: &mut ItemList<QualityItem, InventoryFilter>) {
    ...
    let quality_items: Vec<QualityItem> = items
        .into_iter()
        .map(|inv_item| {
            let at_max = inv_item.item.quality == ItemQuality::Mythic;
            let next_quality = if at_max {
                None
            } else {
                inv_item.item.quality.next_quality()
            };
            let can_afford = player_stones >= STONES_REQUIRED_PER_UPGRADE;
            QualityItem {
                inv_item,
                next_quality,
                can_afford,
...
pub fn render(frame: &mut Frame, area: Rect, item_list: &mut ItemList<QualityItem, InventoryFilter>) {
    ...
    let selected_item = item_list.selected_item().map(|qi| &qi.inv_item.item);
