...
impl Blacksmith {
    ...
    pub fn upgrade_item_quality(&self, player: &mut Player, item: &mut Item) -> Result<(), BlacksmithError> {
        ...
        let stone = player.find_item_by_id(ItemId::QualityUpgradeStone)
    ...
    pub fn smelt_ore(&mut self, player: &mut Player,  recipe_id: &RecipeId) -> Result<Item, BlacksmithError> {
        if self.fuel_amount <= 0 { return Err(BlacksmithError::NotEnoughFuel)}
    ...
    pub fn upgrade_player_item(&self, player: &mut Player, item_uuid: Uuid) -> Result<(), BlacksmithError> {
        // Check equipped items first
        for slot in EquipmentSlot::all() {
            if let Some(equipped) = player.get_equipped_item(*slot) {
                if equipped.item.item_uuid == item_uuid {
                    if let Some(mut inv_item) = player.inventory_mut().equipment_mut().remove(slot) {
                        let result = self.upgrade_item(player, &mut inv_item.item);
                        player.inventory_mut().equipment_mut().insert(*slot, inv_item);
                        return result;
                    }
                }
            }
    ...
    pub fn upgrade_player_item_quality(&self, player: &mut Player, item_uuid: Uuid) -> Result<(), BlacksmithError> {
        // Check equipped items first
        for slot in EquipmentSlot::all() {
            if let Some(equipped) = player.get_equipped_item(*slot) {
                if equipped.item.item_uuid == item_uuid {
    ...
    pub fn smelt_and_give(&mut self, player: &mut Player, recipe_id: &RecipeId) -> Result<(), BlacksmithError> {
        let item = self.smelt_ore(player, recipe_id)?;
        player.add_to_inv(item).map_err(|_| BlacksmithError::InventoryFull)?;
        Ok(())
    }
}
